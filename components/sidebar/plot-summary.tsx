...
import type { PlotSummary as PlotSummaryType, PlotPoint } from "@/lib/types"

interface PlotSummaryProps {
  content: string
  loadedAnalysisData?: PlotSummaryType
  loadedPlotSuggestions?: string[]
  loadedSelectedSuggestions?: Set<number>
}

export interface PlotSummaryRef {
...
export const PlotSummary = forwardRef<PlotSummaryRef, PlotSummaryProps>(
  ({ content, loadedAnalysisData, loadedPlotSuggestions, loadedSelectedSuggestions }, ref) => {
    ...
    useEffect(() => {
      if (loadedAnalysisData) {
        setSummary(loadedAnalysisData.plotSummary || [])
        setActionableInsights(loadedAnalysisData.actionableInsights || [])
        setMetadata(loadedAnalysisData.metadata || null)
      }
    }, [loadedAnalysisData])

    // Load saved plot suggestions if available
    useEffect(() => {
      if (loadedPlotSuggestions && loadedPlotSuggestions.length > 0) {
        console.log('Loading saved plot suggestions:', loadedPlotSuggestions)
        setSuggestions(loadedPlotSuggestions)
      }
    }, [loadedPlotSuggestions])

    // Load saved selected suggestions if available
    useEffect(() => {
      if (loadedSelectedSuggestions && loadedSelectedSuggestions.size > 0) {
        console.log('Loading saved selected suggestions:', loadedSelectedSuggestions)
        setSelectedSuggestions(loadedSelectedSuggestions)
      }
    }, [loadedSelectedSuggestions])

    // Expose plot data through ref
    useImperativeHandle(ref, () => ({
      getAnalysisData: () => {
        if (summary.length > 0 || actionableInsights.length > 0) {
          return {
            plotSummary: summary,
            actionableInsights,
